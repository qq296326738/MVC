var calendar = {
    ...
    clear: function () {
        ...
        calendar.config.enableDates = null;
    },
    _setDay: function (m, y, d) {
        ...
        var setMon = calendar.config.today.getMonth() + 1;
        var setYear = calendar.config.today.getFullYear();
        var setDay = calendar.config.today.getDate();

        var firstDay = new Date(y + "/" + m + "/" + 1).getDay();
        var calDay = [];
        var _d = 1;
        var lastDay = new Date(y, m, 0).getDate();
        var end = Math.ceil((lastDay + firstDay) / 7);
        for (var i = 0; i < end; i++) {
            calDay.push("<tr>");
            for (var j = 0; j < 7; j++) {
                if (i == 0) {
                    if (firstDay > j) {
                        calDay.push("<td date='' class='disDay'>&nbsp;</td>");
                    }
                    else {
                        var f = calendar.festivals(m, _d);
                        var _dx = new Date(y + "/" + m + "/" + _d).format("yyyy-MM-dd");
                        var price = calendar.config.datePrices[_dx];
                        var dx = calendar.config.datePrices && (!price);
                        if(dx) {
                        	calDay.push("<td date='' class='disDay'>" + calendar._render(_d,price) + "</td>");
                        } 
                        else if (calendar.config.minDay != null && new Date(calendar.config.minDay.replace(/[年月日-]/g, "\/")) > new Date(_dx.replace(/[-]/g, "\/"))) {
                            calDay.push("<td date='' class='disDay' price='"+price.price+"'>" + calendar._render(_d,price) + "</td>");
                        }
                        else if (calendar.config.maxDay != null && new Date(calendar.config.maxDay.replace(/[年月日-]/g, "\/")) < new Date(_dx.replace(/[-]/g, "\/"))) {
                            calDay.push("<td date='' class='disDay' price='"+price.price+"'>" + calendar._render(_d,price) + "</td>");
                        }
                        else if (m == currMon && currDay == _d && currYear == y) {
                            calDay.push("<td date='" + _dx + "' class='_selDay' price='"+price.price+"'>" + calendar._render(f !== ""?f:_d,price) + "</td>");
                        }
                        else if (m == setMon && setDay == _d && setYear == y) {
                            calDay.push("<td date='" + _dx + "' title='" + _d + "号' class='_sday' price='"+price.price+"'>"+calendar._render('今天',price)+"</td>");
                        }
                        else if (f !== "") {
                            calDay.push("<td date='" + _dx + "' class='festival' price='"+price.price+"'>" + calendar._render(f,price) + "</td>");
                        }
                        else {
                            calDay.push("<td date='" + _dx + "' price='"+price.price+"'>" + calendar._render(_d,price) + "</td>");
                        }
                        _d++;
                    }
                }
                    ...
                    if (_d <= lastDay) {
                        ...
                        if(dx) {
                            ...
                            calDay.push("<td date='" + _dx + "' title='" + m + "-" + _d + "' class='festival' price='"+price.price+"'>" + calendar._render(f,price) + "</td>");
                        }
                        else {
                            calDay.push("<td date='" + _dx + "' price='"+price.price+"'>" + calendar._render(_d,price) + "</td>");
                        }
    ...
    _render: function(t,d) {
    	if(d) {
    		return "<dl><dt>"+t+"</dt><dd class='price'>￥"+d.price+"</dd></dl>";
    	} else {
    		return "<dl><dt>"+t+"</dt><dd class='price'></dd></dl>";
    	}
    },
    L: function (e) {
    ...
    init: function (s) {
        ...
        _div.setAttribute('style', "left:" + calLeft + "px;top:" + (calTop+1) + "px;z-index:" + calendar.config.zIndex + ";");
    ...
    _bindEvent: function(s) {
    	$("#_tdCal tr:gt(0) td[date!='']").bind('click', function () {
            var date = new Date($(this).attr("date").replace(/[-]/g, "/")).format("yyyy-MM-dd");
            s.value = new Date($(this).attr("date").replace(/[-]/g, "/")).format(calendar.config.format);
            $(s).attr("date", date);
            calendar.colse();
            calendar.config.ok(this);
