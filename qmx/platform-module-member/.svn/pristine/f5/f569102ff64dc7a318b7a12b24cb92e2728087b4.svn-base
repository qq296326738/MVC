...
<form id="form" action="toExchangeSuccess" method="post">
            ...
            <div class="member-part">
                <div class="member-flex member-row">
                    ...
                    <div class="member-flex-main">
                        ...
                        <input id="moneys" <#--name="integral"--> type="hidden" value="${data.money!}"/>
                ...
                <div class="member-flex member-row member-border-none">
                    ...
                    <div class="member-flex-main">
                        <p id="balance">${data.integeral!}积分<#if data.money gt 0> + ${data.money!}元</#if></p>
                    </div>
                </div>
            </div>
            <div class="member-part">
    ...
    <script>
        ...
        $("#submit").click(function () {
            ...
            var text = $("#balance").text();
            var indexOf2 = text.indexOf("积分");
            var integeral = text.substring(0, indexOf2);
            var indexOf = text.indexOf("+");
            var money = 0;
            if (indexOf != -1) {
                money = text.substring(indexOf + 2, $("#balance").text().length - 1);
            }
            var name = $("#name").val();
        ...
        function priceSum() {
            ...
            var moneys = parseFloat($('#moneys').attr('value'));
            integal = num * integal;
            moneys = num * moneys;

            if (moneys > 0) {
                $('#balance').text(integal + '积分 + ' + moneys + '元');
            } else {
