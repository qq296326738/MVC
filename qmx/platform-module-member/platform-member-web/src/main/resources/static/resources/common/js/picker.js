    ...
    Picker.fn.render = function() {
        ...
        var config = that.config,
            ...
            getAreaCodeByCode = function(options) {
                ...
                if(options.type == undefined){
                    options.type = getTypeByCode(options.code);
                }
                for (var i = 0; i < data.length; i++) {
                    var e = data[i];
            ...
            getTypeByCode = function (pCode) {
                //判断pType
                var pType = 1;
                if(pCode != null){
                    var codeStr = pCode.toString();
                    //var codeStr = "abcdef";
                    if(codeStr.length == 6){
                        var p1 = codeStr.substring(2,6);
                        var p2 = codeStr.substring(4,6);
                        if(p1.toString() == '0000'){
                            pType = 1;
                        }else if(p2 == '00'){
                            pType = 2;
                ...
                console.info("getTypeByCode:"+pType);
                return pType;
            },
            ...
            renderData = function(type, $picker, parentCode, selectCode, init) {
                ...
                layui.laytpl(tempHtml).render(getDatas(type, parentCode, selectCode), function(html) {
                    ...
                    form.on('select(' + filter + ')', function(data) {
                        console.info('areaCode:'+data.value);
        ...
        if (config.codeConfig) {
            ...
            switch (pType) {
                case pickerType.province:
                    //渲染省
                    console.info('province:'+pickerType.province);
                    renderData(pickerType.province, $picker, null, arrPath[0]);
                    break;
                case pickerType.city:
                    //渲染省
                    console.info('city:'+pickerType.city);
                    renderData(pickerType.province, $picker, null, arrPath[0]);
                    //渲染市
