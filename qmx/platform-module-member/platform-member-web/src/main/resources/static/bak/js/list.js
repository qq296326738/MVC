...
$().ready( function() {
	...
	$deleteButton.click( function() {
		...
		$.dialog({
			type: "warn",
			content: "您确定要删除"+$checkedIds.size()+"条数据吗？",
	...
	$editButton.click(function() {
		...
		if ($idsCheckedCheck.size() != 1) {
			...
			location.href = "edit.jhtml?id="+ $idsCheckedCheck.val();
		}
	});
	...
	$contentRow.click(function(event){
		var $idsCheck = $(this).find("input[name='ids']:enabled");
		if ($idsCheck.prop("checked")) {
			$idsCheck.prop("checked", false);
			$idsCheck.closest("tr").removeClass("selected");
			...
			$idsCheck.closest("tr").addClass("selected");
			$deleteButton.removeClass("disabled");
		}
	});
	...
	// 列表查询
	if (location.search != "" && filename == "list.jhtml") {
