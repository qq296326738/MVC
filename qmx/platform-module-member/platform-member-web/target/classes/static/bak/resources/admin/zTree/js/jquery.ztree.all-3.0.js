...
(function($){
	...
	_bindEvent = function(setting) {
		...
		o.unbind(c.);
		o.bind(c., function (event, treeId, node) {
			tools.apply(setting.callback.on, [event, treeId, node]);
		});
			...
			switch (nodeEventType) {
				case "switchNode" :
					if (!node.isParent) {
						...
						|| (tools.eqs(event.type, "dblclick") && tools.apply(setting.view.dblClick, [setting.treeId, node], setting.view.dblClick))) {
	...
	handler = {
		onSwitchNode: function (event, node) {
			...
			if (node.open) {
				...
				data.getRoot(setting).TriggerFlag = true;
				view.switchNode(setting, node);
			} else {
				if (tools.apply(setting.callback.before, [setting.treeId, node], true) == false) return true;
				data.getRoot(setting).TriggerFlag = true;
				view.switchNode(setting, node);
			}
			return true;
	...
	view = {
		addNodes: function(setting, parentNode, newNodes, isSilent) {
			...
			if (parentNode) {
				...
				if (!isSilent) {
					view.CollapseParentNode(setting, parentNode, true);
				}
			} else {
		...
		CollapseNode: function(setting, node, Flag, animateFlag, callback) {
			var root = data.getRoot(setting),
			childKey = setting.data.key.children;
			if (!node) {
				tools.apply(callback, []);
			...
			if (root.TriggerFlag) {
				...
				callback = function(){
					...
					if (node.open) {
						setting.treeObj.trigger(consts.event., [setting.treeId, node]);
					} else {
						setting.treeObj.trigger(consts.event.COLLAPSE, [setting.treeId, node]);
					}
				};
				root.TriggerFlag = false;
			}
			if (node.open == Flag) {
				tools.apply(callback, []);
				return;
			...
			if (node.isParent) {
				...
				if (node.open) {
					...
					if (animateFlag == false || setting.view.Speed == "") {
						ulObj.show();
						tools.apply(callback, []);
					} else {
						if (node[childKey] && node[childKey].length > 0) {
							ulObj.slideDown(setting.view.Speed, callback);
						} else {
							ulObj.show();
							tools.apply(callback, []);
						}
					...
					if (animateFlag == false || setting.view.Speed == "") {
						...
						ulObj.slideUp(setting.view.Speed, callback);
					}
				}
			} else {
				tools.apply(callback, []);
		...
		CollapseParentNode: function(setting, node, Flag, animateFlag, callback) {
			if (!node) return;
			if (!node.parentTId) {
				view.CollapseNode(setting, node, Flag, animateFlag, callback);
				return;
			} else {
				view.CollapseNode(setting, node, Flag, animateFlag);
			}
			if (node.parentTId) {
				view.CollapseParentNode(setting, node.getParentNode(), Flag, animateFlag, callback);
			}
		},
		CollapseSonNode: function(setting, node, Flag, animateFlag, callback) {
			var root = data.getRoot(setting),
			childKey = setting.data.key.children,
			treeNodes = (node) ? node[childKey]: root[childKey],
			selfAnimateSign = (node) ? false : animateFlag,
			TriggerFlag = data.getRoot(setting).TriggerFlag;
			data.getRoot(setting).TriggerFlag = false;
			if (treeNodes) {
				for (var i = 0, l = treeNodes.length; i < l; i++) {
					if (treeNodes[i]) view.CollapseSonNode(setting, treeNodes[i], Flag, selfAnimateSign);
				}
			}
			data.getRoot(setting).TriggerFlag = TriggerFlag;
			view.CollapseNode(setting, node, Flag, animateFlag, callback );
		},
		...
		switchNode: function(setting, node) {
			...
			if (node.open || (node && node[childKey] && node[childKey].length > 0)) {
				view.CollapseNode(setting, node, !node.open);
	...
	$.fn.zTree = {
		...
		init: function(obj, zSetting, zNodes) {
			...
			var zTreeTools = {
				...
				All : function(Flag) {
					Flag = !!Flag;
					view.CollapseSonNode(this.setting, null, Flag, true);
					return Flag;
				},
				Node : function(node, Flag, sonSign, focus, callbackFlag) {
					...
					if (callbackFlag && Flag && (tools.apply(setting.callback.before, [setting.treeId, node], true) == false)) {
						return null;
					} else if (callbackFlag && !Flag && (tools.apply(setting.callback.beforeCollapse, [setting.treeId, node], true) == false)) {
						return null;
					}
					if (Flag && node.parentTId) {
						view.CollapseParentNode(this.setting, node.getParentNode(), Flag, false);
					}
					if (Flag === node.open && !sonSign) {
						return null;
					}
					
					data.getRoot(setting).TriggerFlag = callbackFlag;
					if (sonSign) {
						view.CollapseSonNode(this.setting, node, Flag, true, function() {
				...
				selectNode : function(node, addFlag) {
					...
					if (tools.uCanDo(this.setting)) {
						...
						if (node.parentTId) {
							view.CollapseParentNode(this.setting, node.getParentNode(), true, false, function() {
...
(function($){
	...
	_zTreeTools = function(setting, zTreeTools) {
		...
		zTreeTools.editName = function(node) {
			...
			view.CollapseParentNode(setting, node, true);
				...
				if (root.dragFlag == 0) {
					...
					for (i=0, l=nodes.length; i<l; i++) {
						...
						if (tmpNode.isParent && tmpNode.open) {
							view.CollapseNode(setting, tmpNode, !tmpNode.open);
								...
								if (startTimer) {
									window.zTreeMoveTimer = setTimeout(function() {
										...
										if (tmpTargetNode && tmpTargetNode.isParent && !tmpTargetNode.open && (new Date()).getTime() - startTime > targetSetting.edit.drag.autoOpenTime
											...
											if (targetSetting.edit.drag.autoTrigger) {
												targetSetting.treeObj.trigger(consts.event., [targetSetting.treeId, tmpTargetNode]);
											}
										}
	...
	_view = {
		...
		moveNode: function(setting, targetNode, node, moveType, animateFlag, isSilent) {
			...
			if (targetNodeIsRoot) {
				...
				if (moveType == consts.move.TYPE_INNER) {
					view.CollapseNode(setting, targetNode, true, false);
				} else {
					view.CollapseNode(setting, targetNode.getParentNode(), true, false);
				}
			...
			// parents after move
			if (!isSilent) {
				view.CollapseParentNode(setting, node.getParentNode(), true, animateFlag);
			}
		},
